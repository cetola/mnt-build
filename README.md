...
There will also be a link to an image generated by `image-gen.sh` which installs all of these along with an Arm Linux Arm filesystem in a 110G sparse image & bmap file `mnt-reform-[ver]-aarch64.img.zst[.bmap]`. See [the docs](https://github.com/yoctoproject/bmaptool) for more info on bmap-tool and see [Images](#floppy_disk-images) for details on which images you can download.
...
- Image name: mnt-reform-<kernel-version>-aarch64.img (currently 6.19.9)
- Layout (DOS partition table):
  - BOOT ext4 partition (1 GiB)
  - ROOT ext4 partition (rest of disk; sparse image is 110 GiB logical size)
- Installs:
  - ArchLinuxARM base rootfs
    - linux-mnt-reform kernel
    - mnt-reform-qcacld2
    - mnt-reform-lpc
    - reform-tools
