...
There will also be a link to an image generated by `image-gen.sh` which installs all of these along with an Arm Linux Arm filesystem in a 110G sparse image & bmap file `mnt-reform-[ver]-aarch64.img.zst[.bmap]`. See [the docs](https://github.com/yoctoproject/bmaptool) for more info on bmap-tool and see [Images](#images) for details on which images you can download.
